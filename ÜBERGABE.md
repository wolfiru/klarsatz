# Übergabe – Stand 20.09.2026, Version 0.11.2
...
- **Versionsverwaltung:** seit 19.09.2026 ein Git-Repository in `/home/pi/klarsatz` (Zweig `main`).
  ...
  Versionssprünge sind als `vX.Y.Z` markiert, zuletzt `v0.11.2`.
...
**Seit 0.11.2 kommt die Spielwiese in die Lektion** (`assets/kurs.js`): Der Knopf „Hier
...
| Rückweg von den vier Seiten oben: Brotkrümel-Leiste; **seit 0.11.1** zusätzlich dieselbe Kapitel-Seitenleiste wie in der Doku (`.doku-layout`) zum Springen zwischen und innerhalb der vier Seiten | jeweils im Kopf von `einordnung.html`/`programme.html`/`fuerwen.html`/`fragen.html`; `tests/test_browser.py` |
| Eignungstabelle: **seit 0.11.2 Textstufen statt 1–10-Punkte** (externe Kritik: sah nach Messung aus, obwohl daneben "kein Messergebnis" stand — verloren ging nichts, die Zahlen unterschieden ohnehin nie mehr als die drei Farbklassen) | `fuerwen.html`, README.md; `tests/test_zielgruppen.py` hält beide synchron |
...
### 4. Übungsaufgaben mit Selbstprüfung — **gebaut am 20.09.2026 (0.11.2)**
